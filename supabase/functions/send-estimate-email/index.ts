...
function renderPaymentInstructionsHTML(pi: Record<string, any> | null, primaryColor: string, heading = 'Payment Methods'): string {
  if (!pi) return ''
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isSuperAdmin && (!callerProfile || callerProfile.company_id !== estimate.company_id)) {
