...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 5. Verify caller is in the same company (admin or super admin). A verified
    //    super admin bypasses the company match (so impersonated sends succeed);
    //    everyone else stays strictly scoped to their own company.
    ...
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px;">
        ...
        ${totalRow}
        ${dueHtml}
        ${renderPaymentInstructionsHTML(company?.payment_instructions, tenantPrimary, 'Payment Methods', reviewUrl)}
