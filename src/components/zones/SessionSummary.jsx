...
export default function SessionSummary({ zones }) {
  ...
  const cards = [
    sfZones.length    > 0 && { label: t('blueprint:summary.sqFt'),  value: String(parseFloat(totalSF.toFixed(2))), color: TYPE_COLORS.SF },
    lfZones.length    > 0 && { label: t('blueprint:summary.linear'),  value: formatLF(totalLF),            color: TYPE_COLORS.LF    },
    countZones.length > 0 && { label: t('blueprint:summary.items'),   value: String(Math.round(totalCount)), color: TYPE_COLORS.count },
  ].filter(Boolean)
