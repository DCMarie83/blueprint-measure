...
const CATEGORIES = [
  { value: 'material', labelKey: 'time:expenses.category.material' },
  { value: 'labor', labelKey: 'time:expenses.category.labor' },
  { value: 'subcontractor', labelKey: 'time:expenses.category.subcontractor' },
  { value: 'equipment', labelKey: 'time:expenses.category.equipment' },
  { value: 'permit', labelKey: 'time:expenses.category.permit' },
  { value: 'other', labelKey: 'time:expenses.category.other' },
]

const CAT_LABEL_KEY = Object.fromEntries(CATEGORIES.map(c => [c.value, c.labelKey]))
...
export default function ExpensesSection({ projectId, companyId, isAdmin }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      alert(t('time:expenses.saveFailed', { error: err.message || t('common:misc.unknownError') }))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleDelete(id) {
    try {
      ...
      alert(t('time:expenses.deleteFailed', { error: err.message || t('common:misc.unknownError') }))
    }
  }
    ...
    <section style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>
          {t('time:expenses.heading', { count: expenses.length })}{' '}
        ...
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: 16, marginBottom: 12 }}>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 130 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)' }}>{t('time:expenses.fieldCategory')}</label>
              <select
                value={form.category}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 2, minWidth: 180 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)' }}>{t('time:expenses.fieldDescription')}</label>
              <input
                value={form.description}
                onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                placeholder={t('time:expenses.descriptionPlaceholder')}
                style={{ padding: '7px 10px', fontSize: 13, border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', background: 'var(--color-bg)', color: 'var(--color-text)', outline: 'none' }}
              />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 100 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)' }}>{t('time:expenses.fieldAmount')}</label>
              <input
                type="number"
                step="0.01"
                min="0"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 130 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)' }}>{t('time:expenses.fieldDate')}</label>
              <input
                type="date"
          ...
          <p style={{ fontSize: 12, color: 'var(--color-text-muted)', margin: '0 0 12px' }}>
            {t('time:expenses.hint')}
          </p>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minWidth: 140 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)' }}>{t('time:expenses.fieldVendor')}</label>
              <input
                value={form.vendor}
                onChange={e => setForm(f => ({ ...f, vendor: e.target.value }))}
                placeholder={t('time:expenses.vendorPlaceholder')}
                style={{ padding: '7px 10px', fontSize: 13, border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', background: 'var(--color-bg)', color: 'var(--color-text)', outline: 'none' }}
              />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 2, minWidth: 180 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)' }}>{t('time:expenses.fieldNotes')}</label>
              <input
                value={form.notes}
                onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                placeholder={t('time:expenses.notesPlaceholder')}
                style={{ padding: '7px 10px', fontSize: 13, border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', background: 'var(--color-bg)', color: 'var(--color-text)', outline: 'none' }}
              />
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            >
              {saving ? t('time:expenses.savingDots') : editingId ? t('time:expenses.update') : t('common:action.save')}
            </button>
          </div>
        </div>
      )}
      ...
      {expenses.length === 0 && !showForm ? (
        <p style={{ color: 'var(--color-text-muted)', fontSize: 14 }}>{t('time:expenses.empty')}</p>
      ) : (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 600, padding: '1px 7px', borderRadius: 9999, background: 'var(--color-surface-2, rgba(46,139,255,0.08))', color: 'var(--color-primary)', whiteSpace: 'nowrap', textTransform: 'capitalize' }}>
                  {CAT_LABEL_KEY[exp.category] ? t(CAT_LABEL_KEY[exp.category]) : exp.category}
                </span>
