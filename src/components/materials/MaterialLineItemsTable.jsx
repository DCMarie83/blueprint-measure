    ...
    <div style={{ borderTop: '1px solid var(--color-border)', padding: '12px 8px' }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '2 1 200px', minWidth: 180 }}>
          <input style={{ ...cellInput, fontSize: 'var(--text-base)', fontWeight: 600 }} placeholder="Description" value={it.description ?? ''} disabled={readOnly} onChange={e => onUpdate(it.id, { description: e.target.value })} />
          {it.source_zone_name && <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 4 }}>From {it.source_zone_name}</div>}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6 }}>
            ...
            <input aria-label="Unit" placeholder="unit" value={unit} disabled={readOnly}
        ...
        <div style={{ flex: '1 1 220px', minWidth: 200 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 8 }}>
            ...
            <input style={{ ...cellInput, fontVariantNumeric: 'tabular-nums' }} type="number" step="0.01" placeholder="Cost" value={it[`cost_${selectedGrade}`] ?? ''} disabled={readOnly} onChange={e => onUpdate(it.id, { [`cost_${selectedGrade}`]: e.target.value })} />
          </div>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          <div style={{ textAlign: 'right', minWidth: 92 }}>
            <div style={capsLabel}>Line total</div>
      ...
      {adjustOpen && (
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap', marginTop: 10, padding: '10px 8px', background: 'rgba(38,70,76,0.03)', borderRadius: 'var(--radius-sm)' }}>
          {coverage && (
            <Stepper label="Coats" value={it.coats} step={1} min={1} max={5} disabled={readOnly} title="Coats (1–5)" onChange={v => onUpdate(it.id, { coats: v })} />
          )}
      ...
      {sections.map(sec => (
        <div key={sec.category} style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, padding: '4px 8px', borderBottom: '2px solid rgba(38,70,76,0.16)' }}>
            <span style={capsLabel}>{sec.category}</span>
            <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>· {sec.items.length} item{sec.items.length === 1 ? '' : 's'}</span>
          </div>
          {sec.items.map(it => (
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '8px 8px 0', fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', fontVariantNumeric: 'tabular-nums' }}>
            Section subtotal: {money(gradeTotal(sec.items, selectedGrade))}
          </div>
        </div>
      ))}
