...
function blockReason(user, company, isSuperAdmin) {
  ...
  // trial_expired (cron-set): always block
  if (status === 'trial_expired') return 'trial_expired'

  // trialing: belt-and-suspenders check (in case cron hasn't flipped yet)
  if (status === 'trialing') {
  ...
  if (['active', 'pilot'].includes(status)) return null
  if (['suspended', 'canceled'].includes(status)) return status

  // past_due: 3-day grace from status change, then block
  if (status === 'past_due') {
  ...
  // paused: same 3-day grace as past_due
  if (status === 'paused') {
    const changedAt = company.subscription_status_changed_at
    if (!changedAt) return null
    const days = (Date.now() - new Date(changedAt).getTime()) / (1000 * 60 * 60 * 24)
    return days > PAST_DUE_GRACE_DAYS ? 'paused' : null
  }
